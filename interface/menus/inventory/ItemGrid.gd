...
func initialize(item_buttons):
	...
	for button in get_children():
		button.connect("tree_exited",Callable(self,"_on_ItemButton_tree_exited").bind(button))
	get_child(0).grab_focus()
	add_child(placeholder_button)

func _on_ItemButton_tree_exited(button):
	var to_focus_path = button.focus_neighbor_left if button.get_index() > 0 else button.focus_neighbor_right
...
func update_focus_neighbours(ignore=null):
	...
	if ignore:
		buttons_to_update.remove_at(ignore.get_index())
	var count = buttons_to_update.size()
	var index = 0
	for button in buttons_to_update:
		var index_previous = index - 1
		var index_next = (index + 1) % count
		button.focus_neighbor_left = button.get_path_to(buttons_to_update[index_previous])
		button.focus_neighbor_right = button.get_path_to(buttons_to_update[index_next])
		index += 1
