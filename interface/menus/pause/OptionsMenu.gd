...
func set_focus_neighbors():
	...
	for slider in sliders:
		...
		if index_previous >= 0:
			slider.focus_neighbor_top = slider.get_path_to(sliders[index_previous])
		var index_next = index + 1
		if index_next < sliders.size():
			slider.focus_neighbor_bottom = NodePath(slider.get_path_to(sliders[index_next]))
		index += 1

	var back_button = $Column/GoBackButton
	var last_slider = sliders[-1]
	last_slider.focus_neighbor_bottom = last_slider.get_path_to(back_button)
	back_button.focus_neighbor_top = back_button.get_path_to(last_slider)
