...
func _ready():
	options_button.connect('pressed',Callable(self,'open_sub_menu').bind(options_menu))
	save_button.connect('pressed',Callable(self,'open_sub_menu').bind(save_menu))
...
func initialize(args={}):
	...
	items_button.connect('pressed',Callable(self,'open_sub_menu').bind(InventoryMenu, {'inventory':inventory}))
	actor.connect('died',Callable(self,'_on_Player_died'))
...
func open_sub_menu(menu, args={}):
	var last_focused_item = get_viewport().gui_get_focus_owner()
	buttons_container.hide()
	await .open_sub_menu(menu, args).completed
