...
func do_tactic(aim):
	is_attacking=true
	Common.print_with_time("do a tactic")
#	var timer:SceneTreeTimer = get_tree().create_timer(owner.property.attack.time)
	owner.tactic(aim)
	await owner.attack_finished
	state_machine.transit_to("attack", {"destination":owner.destination, "target":target})
...
#				print("> " + d["name"])
		...
		if enemies:
			if owner.is_in_cell_center():
				...
				do_tactic(enemies[0])# set the first enemy as default
				return
		else:
